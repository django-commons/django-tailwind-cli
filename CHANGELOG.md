...
### 🐛 Bug Fixes
- **A `TAILWIND_CLI_PATH` pointing straight at a binary no longer ignores `TAILWIND_CLI_VERSION`**: that filename carries no version, so a bump used to reuse the old binary in silence. The version is read from the binary now and a mismatch warns. The file itself is left alone — you put it there, so replacing it is not the library's call.
